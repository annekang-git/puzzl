...
| 필드 | 의미 | 산출 방식 |
|------|------|----------|
| **`price`** | **판매가 (PUZZL 공급가)** | 환율 + 마진 + 원산지 요율 + 금액대별 요율이 반영된 금액. 판매/정산 기준 |
| **`retailPrice`** | **정가 (브랜드 MSRP)** | 브랜드 권장소비자가(EUR)를 환율만 적용하여 환전한 금액. 할인율/정가 표시용 |

> 일반적인 활용 예시:
> - 상품 페이지: `retailPrice`를 정가(취소선)로 표시, `price`를 판매가로 표시
> - 할인율 계산: `(1 - price / retailPrice) × 100` (%)

### 6-2. 사이즈별 가격
- 같은 상품이라도 사이즈에 따라 `sizes[].price`, `sizes[].retailPrice`가 다를 수 있습니다.
- 결제 시 반드시 **선택한 사이즈의 `sizes[].price`**를 사용해주세요.

### 6-3. 갱신 주기
- 가격/재고 변동은 매일 데이터 갱신 시점(오전 07:13 KST)에 자동 반영됩니다.
- 재고는 `sizes[].stock` 필드로 확인하며, **실시간이 아닌 일 단위 스냅샷**입니다.

### 6-4. 통화
- 모든 가격 필드의 통화는 `currency: "KRW"`로 고정됩니다.
- EUR/USD 원본가는 API 응답에 포함되지 않습니다.
...
| 버전 | 날짜 | 변경 내역 |
|------|------|----------|
| 1.1 | 2026-04-17 | `retailPrice` 필드 추가 (정가/MSRP), 이미지 프록시 엔드포인트 추가 |
